    ...
    public Receive<Command> createReceive() {
        return newReceiveBuilder()
                ...
                .onMessage(PositionCommand.class, message -> {
                    determineNextSpeed();
                    this.currentPosition += getDistanceMovedPerSecond();
                    if (currentPosition > raceLength) {
                        currentPosition = raceLength;
                    }
                    message.getController().tell(new RaceController.RacerUpdateCommand(getContext().getSelf(), (int) this.currentPosition));
                    return Behaviors.same();
                })
                .build();
    }
